...
ws = ws_api.WsApiConnection(True)
...
def load_trading_data() -> pd.DataFrame:
    ...
    _trading_df["totalAmount"] = (
        _trading_df["executionSize"] * _trading_df["executionPrice"]
    ) + _trading_df["executionFee"]
...
def get_user_activity(
    ...
    """
    # Get banking data
    banking_data = get_banking_data(start_date=start_date, end_date=end_date)
    ...
    result = {"banking": banking_data, "trading": trading_data}
...
def calculate_account_balance(
    ...
    balance_df["signed_amount"] = balance_df.apply(
        lambda row: row["amount"] if row["side"] == "CREDIT" else -row["amount"], axis=1
    )
...
def summarize_trading_by_isin(
    ...
    # Add net position
    buy_positions = (
        trading_df[trading_df["direction"] == "BUY"]
        .groupby("ISIN")["executionSize"]
        .sum()
    )
    sell_positions = (
    ...
    summary["average_price"] = (
        summary["total_amount"] - summary["trade_count"]
    ) / summary["total_size"]
...
def get_monthly_summary(
    ...
    banking_df["year_month"] = pd.to_datetime(banking_df["bookingDate"]).dt.to_period(
        "M"
    )
    ...
    # Calculate net flow
    monthly_summary["net_flow"] = (
        monthly_summary["credit_sum"] - monthly_summary["debit_sum"]
    )
    monthly_summary["total_count"] = (
        monthly_summary["credit_count"] + monthly_summary["debit_count"]
    )
